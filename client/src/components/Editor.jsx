...
export const CodeEditor = ({ setLoading }) => {
  const [code, setCode] = useState(languageSnippets.javascript);
  const editorRef = useRef(null);
  const outputRef = useRef(null); // Add ref for Output component
  ...
  const onMount = (editor, monaco) => {
    ...
    editor.addCommand(
      ...
      () => {
        // Call the runCode function from Output component
        if (outputRef.current && outputRef.current.runCode) {
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-6">
      ...
      <div className="w-full lg:w-2/5 ">
        <Output language={language} editorRef={editorRef} ref={outputRef} />
      </div>
    </div>
  );
};
